...
import { filterByMake, filterByModel } from "./filters";
...
export const searchHelpers = {
  ...
  doSearchWithCache: async (
    ...
    if (!search) {
      ...
      // const searchAlbum = state.searchAlbumRelation.filter(
      //   (relation) => relation.searchId === search.id
      // );
      // const needsRefresh = searchAlbum.reduce(
      //   (previousValue, relation) =>
      //     previousValue ||
      //     (state.searches[relation.searchId]?.lastUpdatedPhoto ?? 0) <
      //       (state.albums[relation.albumId]?.lastUpdatedPhoto ?? 0),
      //   false
      // );
      //
      // console.log(state);
      ...
      //
      // if (needsRefresh) {
      //   // Remove search entry and recall this function
      //   await searchRepository.delete(search.id);
      //   return searchHelpers.doSearchWithCache(
      //     connection,
      //     store,
      //     args,
      //     start,
