...
import { NexusGenInputs } from "@howdypix/graphql-schema/schema";
...
import { Events, EventTypes } from "../lib/eventEmitter";
...
function isPhotoMatching(photo: Photo, search: Search): boolean {
  let matching = true;
  ...
  filterBy.make?.forEach((make) => {
    matching = matching && photo.make === make;
  });
...
export async function onNewPhotoEntry(
  ...
  // Check if the photo matches a search
  const searches = await connection.getRepository(Search).find();

  Promise.all(
    searches.map(async (search) => {
      if (isPhotoMatching(photo, search)) {
        ...
        await connection.getRepository(SearchResult).save(searchResult);
      }
    })
  );
}
